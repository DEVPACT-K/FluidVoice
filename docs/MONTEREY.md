...
3. Build unsigned (preserves Accessibility grant):
   ...
   ```
4. Launch, grant Mic + Accessibility — **Whisper Tiny** is now default on all Monterey Intel (tested via `MontereyDefaultModelTests`) for fast clean dictate. Apple Speech also works. Parakeet/Nemotron are auto-blocked on Intel with a clear error.
